...
export default function BrandDetailPage() {
  ...
  const navItems = [
    { label: 'Tasks',     desc: 'Kanban board — track work through the workflow', href: `/brands/${brandId}/tasks` },
    { label: 'Analytics', desc: 'SOW progress and performance breakdown',          href: `/brands/${brandId}/analytics` },
  ]
          ...
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all border-none cursor-pointer ${
                activeTab === tab.id ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-500 bg-transparent hover:text-gray-700'
              }`}
            >{tab.label}</button>
          ))}
        </div>
        ...
        {activeTab === 'members' && (
          <div className="flex flex-col gap-4">
            <div className="flex justify-between items-center">
              <p className="text-sm text-gray-500">{members.length} member{members.length !== 1 ? 's' : ''} assigned to this brand</p>
              {canManage && (
                <button onClick={() => setShowAssign(!showAssign)}
                  className={`px-3 py-1.5 text-xs font-medium rounded-lg cursor-pointer border transition-all ${
                    showAssign ? 'bg-gray-100 text-gray-700 border-gray-200' : 'bg-indigo-600 text-white border-indigo-600 hover:bg-indigo-700'
                  }`}
            ...
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              {members.length === 0 ? (
                <p className="p-8 text-center text-sm text-gray-400">No members yet.</p>
              ) : (
                <div className="divide-y divide-gray-100">
                  {members.map((m) => (
                    <div key={m._id} className="flex items-center gap-3 px-5 py-4 hover:bg-gray-50 transition-colors">
        ...
        >
          <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-500 mb-6">
              Are you sure you want to delete <span className="font-medium text-gray-900">{brand.name}</span>? This cannot be undone.
            </p>
            <div className="flex gap-3">
              <button onClick={() => setShowDelete(false)} className="flex-1 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg border-none cursor-pointer transition-colors">Cancel</button>
              <button onClick={deleteBrand} disabled={deleting} className="flex-1 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 disabled:bg-red-300 rounded-lg border-none cursor-pointer transition-colors">
