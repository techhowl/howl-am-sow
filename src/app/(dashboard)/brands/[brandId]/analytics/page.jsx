...
export default function BrandAnalyticsPage() {
  ...
  useEffect(() => {
    fetchData();
  }, [period, refDate, brandId]);

  async function fetchData() {
    setLoading(true);
    try {
      const res = await fetch(
        `/api/brands/${brandId}/analytics?period=${period}&date=${refDate.toISOString()}`
      );
  ...
  function handleExportTasks() {
    ...
    exportCSV(
      data.taskList,
      `${data.brand?.name || 'brand'}-tasks-${period}-${format(refDate, 'yyyy-MM-dd')}.csv`
    );
  }
  ...
  function handleExportUsers() {
    ...
    exportCSV(
      ...
      `${data.brand?.name || 'brand'}-team-${period}-${format(refDate, 'yyyy-MM-dd')}.csv`
    );
  }
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {data?.brand && (
              <div className="flex items-center gap-1.5 ml-1">
                ...
                <span className="text-sm text-gray-500">{data.brand.name}</span>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1 border border-gray-200 rounded-lg">
              <button onClick={prev} className="p-1.5 hover:bg-gray-50 rounded-l-lg transition-colors">
                <ChevronLeft className="w-4 h-4 text-gray-500" />
              </button>
              <span className="text-xs font-medium text-gray-700 px-3 min-w-[140px] text-center">
                {getPeriodLabel()}
              </span>
              <button onClick={next} className="p-1.5 hover:bg-gray-50 rounded-r-lg transition-colors">
          ...
          <>
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-5">
              ...
              <div className="flex gap-6">
                {[
                  ...
                  <div key={key} className={`flex-1 rounded-xl px-4 py-3 ${color}`}>
                    <p className="text-xs font-medium opacity-70 mb-1">{label}</p>
                    <p className="text-xl font-bold">{data.priorityBreakdown[key] || 0}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              {data.userStats.length === 0 ? (
                ...
                <div className="rounded-xl border border-gray-100 overflow-hidden">
                  <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] bg-gray-50 px-4 py-2.5 border-b border-gray-100">
                    {['Member', 'Role', 'Assigned', 'Completed', 'Revisions'].map((h) => (
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-5">
              ...
              {data.taskList.length === 0 ? (
                <p className="text-sm text-gray-400">No tasks for this period</p>
              ) : (
                <div className="rounded-xl border border-gray-100 overflow-hidden">
                  <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr] bg-gray-50 px-4 py-2.5 border-b border-gray-100">
                    {['Task', 'Status', 'Priority', 'Revisions', 'Int. Deadline', 'Ext. Deadline'].map((h) => (
                      <p key={h} className="text-xs font-semibold text-gray-400 uppercase tracking-wide">{h}</p>
                    ))}
                  </div>
                  {data.taskList.map((t, i) => (
                    ...
                    >
                      <p className="text-sm font-medium text-gray-900 truncate pr-2">{t.title}</p>
                      <span className="text-xs text-gray-500 self-center capitalize">
                        {STATUS_LABELS[t.status] || t.status}
                      </span>
                      <span className="text-xs text-gray-500 self-center capitalize">{t.priority}</span>
                      <span className="text-sm text-gray-700 self-center">{t.revisionCount}</span>
                      <span className="text-xs text-gray-500 self-center">{t.internalDeadline || '—'}</span>
                      <span className="text-xs text-gray-500 self-center">{t.externalDeadline || '—'}</span>
                    </div>
                  ))}
                </div>
              )}
