        ...
        <div className="flex gap-3 group">
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-muted-foreground capitalize">{author?.role?.replace(/_/g, ' ')}</span>
                ...
                {editing ? (
                    <div className="space-y-2">
                        <textarea
                            ...
                            rows={2}
                            className="w-full border border-border rounded-lg px-3 py-2 text-sm resize-none focus:outline-none focus:border-ring focus:ring-2 focus:ring-ring/30"
                        />
                        <div className="flex gap-2">
                            <button
                                onClick={handleEdit}
                                disabled={saving}
                                className="flex items-center gap-1 text-xs bg-primary hover:bg-primary/90 text-primary-foreground px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50"
                            >
                            ...
                            <button
                                ...
                                className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground px-3 py-1.5 rounded-lg hover:bg-muted transition-colors"
                            >
                                <X className="w-3 h-3" />
                                Cancel
                            </button>
                    ...
                    <div className="bg-muted rounded-xl px-3 py-2.5 text-sm text-foreground leading-relaxed">
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="border-t border-border pt-4 relative">
                {/* Mention suggestions */}
                {showSuggestions && (
                    <div className="absolute bottom-full left-0 right-0 mb-1 bg-card border border-border rounded-xl shadow-lg z-10 overflow-hidden">
                        {suggestions.map((m) => (
                            <button
                                key={m.userId?._id}
                            ...
                            >
                                ...
                                <div>
                                    <p className="text-sm font-medium text-foreground">{m.userId?.name}</p>
                                    <p className="text-xs text-muted-foreground capitalize">{m.userId?.role?.replace(/_/g, ' ')}</p>
                                </div>
                            </button>
                        ))}
                    </div>
                ...
                <div className="flex gap-2 items-end">
                    <textarea
                        ...
                        rows={2}
                        className="flex-1 border border-border rounded-xl px-3 py-2.5 text-sm resize-none focus:outline-none focus:border-ring focus:ring-2 focus:ring-ring/30 placeholder:text-muted-foreground"
                    />
                    <button
                        onClick={handleSubmit}
                        disabled={!content.trim() || submitting}
                        className="p-2.5 bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl transition-colors disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
                        title="Send (Ctrl+Enter)"
                    >
                        <Send className="w-4 h-4" />
                    </button>
                </div>
                <p className="text-xs text-muted-foreground mt-1.5 ml-1">Ctrl+Enter to send</p>
            </div>
        </div>
    );
}